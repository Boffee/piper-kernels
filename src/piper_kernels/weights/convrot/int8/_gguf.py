...
def select_gguf_converter(
    data: torch.Tensor,
) -> Callable[[torch.Tensor, int, int, torch.Tensor, torch.Tensor], None] | None:
    """Select direct GGUF conversion independently of matrix-operation support."""
    if not runtime.supports_device(data.device):
        return None
    from .triton import convert_gguf_out  # noqa: PLC0415

    return convert_gguf_out
...
def convert(
    ...
    *,
    ...
    convert_out = select_gguf_converter(raw)
