...
def _can_use_triton_swiglu(
    ...
    return (
        _triton_swiglu_linear is not None
        and _can_use_triton(activation, qdata)
        and group_size == 256
        and activation.dtype in (torch.float16, torch.bfloat16)
        and torch.cuda.get_device_capability(activation.device) >= (12, 0)
        and rows >= 512
        and qdata.shape[1] <= 16_384
    )
...
def _linear(
    ...
    """
    _validate_linear(activation, qdata, scale, group_size, bias, qdata.shape[1])
...
def _linear_input_act(
    ...
    if input_act != "swiglu":
        raise ValueError(f"ConvRot input_act must be 'swiglu', got {input_act!r}")
    _validate_linear(activation, qdata, scale, group_size, bias, 2 * qdata.shape[1])
    ...
    return _linear(activated, qdata, scale, group_size, bias)
