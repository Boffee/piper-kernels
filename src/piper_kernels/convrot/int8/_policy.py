...
class PreparationPlan:
    ...
    ``fused_block_size`` and ``fused_num_warps`` describe the fused candidate launch,
...
def select_preparation_plan(
    ...
    *,
    ...
    fused_block_size = 128 if in_features <= 0 else max(128, 1 << (in_features - 1).bit_length())
    ...
    fuse_rotation_quantization = (
        ...
        and fused_block_size <= _FUSED_MAX_BLOCK_SIZE
    )
    large_swiglu = tuned_sm120 and swiglu and fused_block_size == _FUSED_MAX_BLOCK_SIZE
    fused_num_warps = (
        16 if large_swiglu and rows >= _LARGE_SWIGLU_MIN_ROWS else 8 if large_swiglu else 4
    )
    return PreparationPlan(
        fused_block_size=fused_block_size,
