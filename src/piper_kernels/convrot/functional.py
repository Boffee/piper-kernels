...
from .int8.tensor import ConvRotInt8Tensor, _convrot_int8_linear
...
def convrot_linear(
    ...
    *,
    ...
    ``activation`` has shape ``[..., 2 * in_features]`` for ``"swiglu"`` and
    uses the raw ``[up | gate]`` layout. ``weight`` has logical shape
    ``[out_features, in_features]`` and ``bias``, when present, has shape
    ``[out_features]``. Activation, logical weight, and bias dtypes and devices
    ...
    return _convrot_int8_linear(activation, weight, bias, input_activation)
