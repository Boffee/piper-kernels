    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            ...
            f"tensor with shape ({features},) on {input.device}"
        )
...
def _validate_inputs(
    ...
    if input.ndim == 0 or input.layout is not torch.strided:
        raise ValueError("chunked ConvRot FFN input must be a non-scalar strided tensor")
...
def _run_chunked_swiglu_ffn(
    ...
    input_2d = input.reshape(rows, input_features).contiguous()
    output = torch.empty(
