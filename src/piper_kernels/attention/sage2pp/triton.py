...
def _causal_attention_tile(
    ...
    if qk_per_warp and raw_score_recurrence:
...
@triton.jit
def _sage_attention_2pp_kernel(  # noqa: PLR0912, PLR0915 - keep the noncausal loop monolithic
    query_ptr,
    key_ptr,
    value_ptr,
    query_scale_ptr,
    ...
    if is_causal:
        # The prefix omits its mask only while every lane is both physically
        # valid and strictly before this query block. A ragged K tail stays in
        # the masked boundary loop even for query blocks beyond key_length.
        ...
        # Keep this body separate from the causal tile helper. Inlining the
        # noncausal loop preserves its lower-spill SM120 register allocation.
...
def _should_use_attention_tensor_descriptors(
    ...
    return device_major == 12 and block_m == 128 and head_dim == 128 and key_length % 16 == 0
...
def _run_sage_attention_2pp(
    ...
    *,
    ...
    storage_key_length = (
        padded_key_length if use_tensor_descriptors and key_length % 16 != 0 else key_length
    )
    ...
    if qk_per_warp:
        # A query tile is consumed by exactly one CTA. Quantizing it in the
        # attention prologue avoids an extra launch and a global INT8 round trip.
        ...
        # This argument is compiled away in the per-warp specialization.
