"""Fixed four-wave Q64/D128 fragments for the RDNA4 attention schedule."""
...
_RESCALE_ASM = gl.constexpr(
    "\n".join(
        ["v_cmp_neq_f32_e64 vcc_lo, 1.0, $128", "s_cbranch_vccz 1f"]
        + [f"v_mul_f32 ${index}, ${index}, $128" for index in range(64)]
        + ["1:"]
    )
)
_RESCALE_CONSTRAINTS = gl.constexpr(
    ",".join(["=&v"] * 64 + [str(index) for index in range(64)] + ["v"] * 64 + ["~{vcc}"])
)
...
def _join_matrix(fragments):
    result = concat_columns(
        ...
        concat_columns(
            concat_columns(fragments[4], fragments[5]), concat_columns(fragments[6], fragments[7])
        ),
    )
...
def query_fragments(query_ptr, blocks, query_storage_length: gl.constexpr):
    ...
    for k in gl.static_range(8):
        fragments += (
            gl.load(words + rows * 16 + (k // 2) * 4 + (lane[None, :] // 16) * 2 + k % 2),
        )
    return fragments


@gluon.jit
def qk_pair(query, key_ptr, tile_0, tile_1, storage_length: gl.constexpr):
    offset_type: gl.constexpr = gl.uint32 if storage_length * 128 <= 4294967296 else gl.int64
    ...
    for group in gl.static_range(4):
        for column in gl.static_range(group * 2, group * 2 + 2):
            ...
            # All D128 INT8 dot products are bounded by 2^21. This integer
            ...
            for k in gl.static_range(8):
                offset = (row * 128 + (k // 2) * 32 + (lane[None, :] // 16) * 16 + (k % 2) * 8).to(
                    offset_type
                )
...
def rescale_numerator(numerator, old_weight):
    ...
    gl.static_assert(numerator.shape == [1, 64, 128])
...
def pv_pair(
    ...
    numerators = eight_fragments(numerator)
    results = ()
    for group in gl.static_range(4):
        for column_tile in gl.static_range(group * 2, group * 2 + 2):
            ...
            for k in gl.static_range(8):
                ...
                offset = (start[:, None] * 128 + column[None, :] * 64 + token).to(offset_type)
