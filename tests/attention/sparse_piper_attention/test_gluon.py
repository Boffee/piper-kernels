...
@gluon.jit
def _paired_pv_kernel(probability_0_ptr, probability_1_ptr, value_0_ptr, value_1_ptr, output_ptr):
    ...
    probability_0 = gl.convert_layout(
        gl.load(probability_0_ptr + probability_offsets), probability_layout
    )
    probability_1 = gl.convert_layout(
        gl.load(probability_1_ptr + probability_offsets), probability_layout
    )
    value_offsets = d[:, None] + k[None, :] * 128
    value_0 = gl.allocate_shared_memory(
        gl.int8, [128, 64], shared_layout, gl.load(value_0_ptr + value_offsets)
    )
    value_1 = gl.allocate_shared_memory(
        gl.int8, [128, 64], shared_layout, gl.load(value_1_ptr + value_offsets)
    )
    gl.barrier()
    output = _piper_pv_pair(
        ...
        gl.full([64], 0.5, gl.float32, gl.SliceLayout(1, mma)),
...
def test_paired_pv_accumulation_matches_int64_products(value_extremes):
    ...
    operands = [tensor.cuda() for tensor in (*probabilities, *values)]
