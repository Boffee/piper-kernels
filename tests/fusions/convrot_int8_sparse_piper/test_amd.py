...
def test_h3_projections_match_sampled_fp64_math_through_100k(sequence):
    ...
    stream.synchronize()
    assert _backend.select_output_backend(q[0]) is None
    ...
    for head in (0, heads // 2, heads - 1):
        ...
        for start in sorted({0, (sequence // 2) // 64 * 64, (sequence - 1) // 64 * 64}):
            ...
            for index, output in enumerate((q, k, v)):
                projected = (
                ...
                if index < 2:
                    check_qk_sample_fp64(
                        projected, output, head, start, rows, norm, cos, sin, is_query=index == 0
                    )
                else:
                    check_value_sample_fp64(projected, output, head, start, rows, mean_v)
