...
import pytest
import torch
from benchmark_convrot_int8 import (
    Result,
    _benchmark_shapes,
...
def test_private_comfy_preparation_adapter_accepts_its_declared_contract(
    ...
    monkeypatch.setattr(
        "benchmark_convrot_int8_preparation.importlib.metadata.version",
        lambda _name: "0.2.28",
    )
    monkeypatch.setattr(
        "benchmark_convrot_int8_preparation.importlib.import_module",
        lambda _name: module,
    )
...
def test_preparation_cli_and_records_expose_phase_timings(tmp_path) -> None:
    ...
    phase = PreparationPhaseResult(
        ...
        operation_provenance=(
            "piper_kernels.linear.convrot.int8._nvidia.triton.fused_rotate_quantize_input"
        ),
