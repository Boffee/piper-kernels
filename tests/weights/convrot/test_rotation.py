...
from piper_kernels.weights.convrot._rotation import build_hadamard
...
def test_fake_rotation_does_not_reuse_or_populate_concrete_cache(warm_cache: bool) -> None:
    _rotation._build_hadamard_cached.cache_clear()
    before = build_hadamard(16) if warm_cache else None
    with FakeTensorMode():
        fake = build_hadamard(16)
        assert isinstance(fake, FakeTensor)
        assert build_hadamard(16) is not fake

    concrete = build_hadamard(16)
    assert not isinstance(concrete, FakeTensor)
    assert build_hadamard(16) is concrete
    if before is not None:
        assert concrete is before
